...
public class BlogApiJsonDirectAccess : IBlogApi
{
    ...
    private void Load<T>(ref List<T>? list, string folder)
    {
        if (list == null)
        {
            ...
            string fullpath = $@"{_settings.DataPath}\{folder}";
    ...
    {
        string filepath = $@"{_settings.DataPath}\{folder}\{filename}";
